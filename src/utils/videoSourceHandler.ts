...
import { isFileReferenceSource } from './mediaSources.js';
...
export async function processVideoSource(
  ...
  // File reference (Gemini Files API) - either a bare "files/..." id or the
  // full "https://generativelanguage.googleapis.com/..." URI returned after an
  // upload. Both must be passed through as fileData, never fetched directly.
  if (isFileReferenceSource(videoSource)) {
    return {
      fileUri: videoSource,
      mimeType: 'video/mp4',
      uploadDuration: 0,
