...
export const ConfigSchema = z.object({
  ...
  MEDIA_RESOLUTION: z
    ...
    .optional(),
  MEDIA_RESOLUTION_FOR_IMAGE: z
    .enum(['low', 'medium', 'high', 'ultra_high'])
    .optional(),
  MEDIA_RESOLUTION_FOR_VIDEO: z
    .enum(['low', 'medium', 'high', 'ultra_high'])
    .optional(),

  // Thinking level (Gemini 3+)
  THINKING_LEVEL: z.enum(['minimal', 'low', 'medium', 'high']).optional(),
  THINKING_LEVEL_FOR_IMAGE: z
    .enum(['minimal', 'low', 'medium', 'high'])
    .optional(),
  THINKING_LEVEL_FOR_VIDEO: z
    .enum(['minimal', 'low', 'medium', 'high'])
    .optional(),
  ...
  MAX_TOKENS_FOR_DETECT_OBJECTS_IN_IMAGE: z
    ...
    .optional()
    .default(8192),
  TEMPERATURE_FOR_SEGMENT_OBJECTS_IN_IMAGE: z
    .number()
    .min(0)
    .max(2)
    .optional()
    .default(0),
  TOP_P_FOR_SEGMENT_OBJECTS_IN_IMAGE: z
    .number()
    .min(0)
    .max(1)
    .optional()
    .default(0.95),
  TOP_K_FOR_SEGMENT_OBJECTS_IN_IMAGE: z
    .number()
    .int()
    .positive()
    .optional()
    .default(30),
  // Segmentation masks are base64 PNGs inside the JSON output, so responses
  // are much larger than detection responses.
  MAX_TOKENS_FOR_SEGMENT_OBJECTS_IN_IMAGE: z
    .number()
    .int()
    .positive()
    .optional()
