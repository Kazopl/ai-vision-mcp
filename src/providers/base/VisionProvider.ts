...
export abstract class BaseVisionProvider implements VisionProvider {
  ...
  protected buildConfigWithOptions(
    ...
    // Media resolution (Gemini 3+): only sent when explicitly requested via
    // runtime option or MEDIA_RESOLUTION[_FOR_IMAGE|_FOR_VIDEO] env vars,
    // since older models reject the parameter.
    // NOTE: 'ultra_high' is only valid at the media Part level (the request
    // level generation_config rejects it), so it is applied via
    // getPartMediaResolution() in the providers instead. The API also rejects
    // ultra_high on video parts, so for video it degrades to 'high' here.
    const mediaResolution =
      options?.mediaResolution ??
      this.configService.getMediaResolutionForTask(taskType);
    if (mediaResolution) {
      let effective = mediaResolution as string;
      if (effective === 'ultra_high' && taskType === 'video') {
        console.error(
          '[VisionProvider] mediaResolution "ultra_high" is not supported for video by the Gemini API; using "high" instead'
        );
        effective = 'high';
      }
      const mapped = {
  ...
  /**
   * Part-level media resolution (Gemini 3+). 'ultra_high' is rejected by the
   * request-level generation_config, so it must be attached to the media Part
   * itself. Lower levels stay at the request level (see buildConfigWithOptions).
   * The API rejects ultra_high on video parts, so it only applies to images;
   * video requests degrade to request-level 'high' in buildConfigWithOptions.
   */
  protected getPartMediaResolution(
    taskType: TaskType,
    options?: AnalysisOptions
    ...
    const mediaResolution =
      options?.mediaResolution ??
      this.configService.getMediaResolutionForTask(taskType);
    if (mediaResolution === 'ultra_high' && taskType === 'image') {
  ...
  protected getThinkingConfig(
    ...
    // Gemini 3.5 series - no forced level: the API default (medium for
    // 3.5 Flash) is the best speed/quality balance for vision tasks, and
    // dense/small-text reads benefit from real thinking. Override via the
    // thinkingLevel option or THINKING_LEVEL env vars.
