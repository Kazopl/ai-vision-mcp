...
export type { CropRegion } from '../utils/imageCrop.js';

/**
 * Load the raw image bytes for a source we can crop locally.
 * File-reference (files/...) and gs:// sources are remote-only and
 * cannot be cropped client-side.
 */
async function loadImageBuffer(
  ...
  if (imageSource.startsWith('data:image/')) {
    return Buffer.from(imageSource.split(',')[1], 'base64');
  }
  if (imageSource.startsWith('http')) {
    const response = await fetch(imageSource);
    if (!response.ok) {
      throw new VisionError(
        `Failed to fetch image for cropping: ${response.status} ${response.statusText}`,
        'FETCH_ERROR'
      );
    }
    return Buffer.from(await response.arrayBuffer());
  }
  return imageFileService.readFile(imageSource);
}

/**
 * Crop the image to the given pixel region (top-left origin) and return it
 * as a PNG data URI. The region is clamped to the image bounds.
 */
async function cropImageToDataUri(
  imageSource: string,
  region: CropRegion,
  imageFileService: FileService
): Promise<string> {
  if (imageSource.startsWith('files/') || imageSource.startsWith('gs://')) {
    throw new VisionError(
      'cropRegion is not supported for file-reference or GCS sources; pass a URL, base64 data, or local file path instead',
      'INVALID_ARGUMENT'
    );
  }

  const buffer = await loadImageBuffer(imageSource, imageFileService);
  const encoded = await cropImageBuffer(buffer, region);
  console.error(
    `[analyze_image] Cropped to region {x: ${region.x}, y: ${region.y}, w: ${region.width}, h: ${region.height}} before analysis`
  );
...
export interface AnalyzeImageArgs {
  ...
  prompt: string;
  mode?: 'general' | 'palette' | 'hierarchy' | 'components';
  cropRegion?: CropRegion; // Crop to this pixel region before analysis
...
export async function analyze_image(
  ...
  try {
    ...
    // Crop first when requested: zooming into the region of interest makes
    // small text far more readable and cuts input tokens.
    const effectiveSource = args.cropRegion
      ? await cropImageToDataUri(
          args.imageSource,
          args.cropRegion,
          imageFileService
        )
    ...
    const processedImageSource =
      await imageFileService.handleImageSource(effectiveSource);
    console.error(
